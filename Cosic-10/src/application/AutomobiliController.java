...
public class AutomobiliController {
	...
	@FXML
	private TableColumn<Automobil, String> stupacNaslov;

	@FXML
	private TableColumn<Automobil, String> stupacOpis;

	@FXML
	private TableColumn<Automobil, String> stupacSnaga;

	@FXML
	private TableColumn<Automobil, String> stupacCijena;
	
	@FXML
	private TableColumn<Automobil, String> stupacStanje;
	...
	@SuppressWarnings("unchecked")
	public void filterAutomobila() throws Exception, BazaPodatakaException {
		String nazivString = naslov.getText();
		String opisString = opis.getText();
		String snagaString = snaga.getText();
		String cijenaString = cijena.getText();
		listaAutomobila = BazaPodataka.dohvatiAutomobilePremaKriterijima(null);
		
		try {
		List<Automobil> filterAutomobila = listaAutomobila.stream().filter(auto -> auto.getNaslov().toLowerCase().contains(nazivString.toLowerCase()))
				.filter(auto -> auto.getOpis().toLowerCase().contains(opisString.toLowerCase()))
				.filter(auto -> auto.getSnagaKs().toString().toLowerCase().contains(snagaString.toLowerCase()))
				.filter(auto -> auto.getCijena().toString().toLowerCase().contains(cijenaString.toLowerCase()))
				.collect(Collectors.toList());
		
		
			data = FXCollections.observableList(filterAutomobila);
