...
public class PoslovniKorisnikController {
		...
		private List<PoslovniKorisnik> listaPoslovnihKorisnika;
		...
		@FXML
		private TableColumn<PoslovniKorisnik, String> stupacNaziv;

		@FXML
		private TableColumn<PoslovniKorisnik, String> stupacWeb;

		@FXML
		private TableColumn<PoslovniKorisnik, String> stupacEmail;

		@FXML
		private TableColumn<PoslovniKorisnik, String> stupacTelefon;
		...
		@SuppressWarnings("unchecked")
		public void filterPoslovnihKorisnika() throws Exception, BazaPodatakaException {
			String nazivString = naziv.getText();
			String webString = web.getText();
			String emailString = email.getText();
			String telefonString = telefon.getText();
			listaPoslovnihKorisnika = BazaPodataka.dohvatiPoslovneKorisnikePremaKriterijima(null);
			
			try {
			List<PoslovniKorisnik> filterPoslovnihKorisnika = listaPoslovnihKorisnika.stream().filter(poslK -> poslK.getNaziv().toLowerCase().contains(nazivString.toLowerCase()))
					.filter(poslK -> poslK.getWeb().toLowerCase().contains(webString.toLowerCase()))
					.filter(poslK -> poslK.getEmail().toLowerCase().contains(emailString.toLowerCase()))
					.filter(poslK -> poslK.getTelefon().toLowerCase().contains(telefonString.toLowerCase()))
					.collect(Collectors.toList());
			
			
				dataPoslovni = FXCollections.observableList(filterPoslovnihKorisnika);
