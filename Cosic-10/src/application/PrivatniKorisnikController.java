...
public class PrivatniKorisnikController {
	...
	private List<PrivatniKorisnik> listaPrivatnihKorisnika;
	...
	@FXML
	private TableColumn<PrivatniKorisnik, String> stupaciIme;

	@FXML
	private TableColumn<PrivatniKorisnik, String> stupacPrezime;

	@FXML
	private TableColumn<PrivatniKorisnik, String> stupacEmail;

	@FXML
	private TableColumn<PrivatniKorisnik, String> stupacTelefon;
	...
	@SuppressWarnings("unchecked")
	public void filterPrivatnihKorisnika() throws Exception, BazaPodatakaException {
		String imeString = ime.getText();
		String prezimeString = prezime.getText();
		String emailString = email.getText();
		String telefonString = telefon.getText();
		listaPrivatnihKorisnika = BazaPodataka.dohvatiPrivatneKorisnikePremaKriterijima(null);
		
		try {
		List<PrivatniKorisnik> filterPrivatnihKorisnika = listaPrivatnihKorisnika.stream().filter(privK -> privK.getIme().toLowerCase().contains(imeString.toLowerCase()))
				.filter(privK -> privK.getPrezime().toLowerCase().contains(prezimeString.toLowerCase()))
				.filter(privK -> privK.getEmail().toLowerCase().contains(emailString.toLowerCase()))
				.filter(privK -> privK.getTelefon().toLowerCase().contains(telefonString.toLowerCase()))
				.collect(Collectors.toList());
		
		
			dataPrivate = FXCollections.observableList(filterPrivatnihKorisnika);
