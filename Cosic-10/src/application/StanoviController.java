...
public class StanoviController {
	...
	private List<Stan> listaStanova;
	...
	@FXML
	private TableColumn<Stan, String> stupacNaslov;

	@FXML
	private TableColumn<Stan, String> stupacOpis;

	@FXML
	private TableColumn<Stan, String> stupacKvadratura;

	@FXML
	private TableColumn<Stan, String> stupacIzvodjac;

	@FXML
	private TableColumn<Stan, String> stupacCijena;

	@FXML
	private TableColumn<Stan, String> stupacStanje;


	@SuppressWarnings("unchecked")
	public void filterStanova() throws Exception, BazaPodatakaException {
		String nazivString = naslov.getText();
		String opisString = opis.getText();
		String kvadraturaString = kvadratura.getText();
		String cijenaString = cijena.getText();
		listaStanova = BazaPodataka.dohvatiStanovePremaKriterijima(null);

		try {
			List<Stan> filterStanova = listaStanova.stream()
					.filter(stan -> stan.getNaslov().toLowerCase().contains(nazivString.toLowerCase()))
					.filter(stan -> stan.getOpis().toLowerCase().contains(opisString.toLowerCase()))
					.filter(stan -> Integer.toString(stan.getKvadratura()).contains(kvadraturaString.toLowerCase()))
					.filter(stan -> stan.getCijena().toString().toLowerCase().contains(cijenaString.toLowerCase()))
					.collect(Collectors.toList());

			dataS = FXCollections.observableList(filterStanova);
