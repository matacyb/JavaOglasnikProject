...
public class UnosAutomobiliController {
	...
	ObservableList<String> stanje = FXCollections.observableArrayList("novo", "izvrsno", "rabljeno", "neispravno");
	...
	@FXML
	public void unosNovogAutomobila() throws BazaPodatakaException, Exception {
		String errorText = new String();
		if(naslov.getText().isBlank() || opis.getText().isBlank() || cijena.getText().isBlank() || stanjeBox.getValue() == null || snaga.getText().isBlank()) {
			...
			if(snaga.getText().isBlank()) {
				errorText += "Snaga je obavezna!!" +System.lineSeparator();
			}
			...
			List<Automobil> listItems = BazaPodataka.dohvatiAutomobilePremaKriterijima(null);
			OptionalLong maxId = listItems.stream().mapToLong(Entitet::getId).max();
			Automobil auto = new Automobil(naslov.getText(), opis.getText(),
					new BigDecimal(cijena.getText()), new BigDecimal(snaga.getText()),(Stanje) stanjeBox.getValue(), maxId.getAsLong() + 1);
			listItems.add(auto);
			BazaPodataka.pohraniNoviAutomobil(auto);
		}
		prikaziEkranZaAutomobile();
	}
