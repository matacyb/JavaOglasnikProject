...
public class UnosPoslovniKorisnikController {
	...
	@FXML
	public void unosNovogPoslovnogKorisnika() throws BazaPodatakaException, Exception {
		String errorText = new String();
		if (naziv.getText().isBlank() || email.getText().isBlank() || web.getText().isBlank()
				|| telefon.getText().isBlank()) {
			if (naziv.getText().isBlank()) {
				errorText += "Naziv je obavezan!" + System.lineSeparator();
			}
			if (email.getText().isBlank()) {
				errorText += "Email je obavezan!" + System.lineSeparator();
			}
			if (web.getText().isBlank()) {
				errorText += "Web je obavezan!" + System.lineSeparator();
			}
			...
			List<PoslovniKorisnik> listaKorisnika = BazaPodataka.dohvatiPoslovneKorisnikePremaKriterijima(null);
			OptionalLong maxId = listaKorisnika.stream().mapToLong(Entitet::getId).max();
			PoslovniKorisnik korisnik = new PoslovniKorisnik(naziv.getText(), web.getText(), email.getText(),
					telefon.getText(), maxId.getAsLong() + 1);
			BazaPodataka.pohraniNovogPoslovnogKorisnika(korisnik);
		}
		prikaziEkranZaPoslovneKorisnike();
	}
