...
public class UnosUslugaController {
	...
	public void unosNoveUsluge() throws BazaPodatakaException, Exception {
		...
		if (naslov.getText().isBlank() || opis.getText().isBlank() || cijena.getText().isBlank()
			...
			List<Usluga> listItems = BazaPodataka.dohvatiUslugePremaKriterijima(null);
			OptionalLong maxId = listItems.stream().mapToLong(Entitet::getId).max();
			Usluga usluga = new Usluga(naslov.getText(), opis.getText(),
					new BigDecimal(cijena.getText()), (Stanje) stanjeBox.getValue(), maxId.getAsLong() + 1);
			BazaPodataka.pohraniNovuUslugu(usluga);
		}
		prikaziEkranZaUsluge();
	}
