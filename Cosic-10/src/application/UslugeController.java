...
public class UslugeController {
	...
	@FXML
	private TableColumn<Usluga, String> stupacNaslov;

	@FXML
	private TableColumn<Usluga, String> stupacOpis;

	@FXML
	private TableColumn<Usluga, String> stupacCijena;

	@FXML
	private TableColumn<Usluga, String> stupacStanje;

	
	
	@SuppressWarnings("unchecked")
	public void filterUsluga() throws Exception, BazaPodatakaException {
		String nazivString = naslov.getText();
		String opisString = opis.getText();
		String cijenaString = cijena.getText();
		listaUsluga = BazaPodataka.dohvatiUslugePremaKriterijima(null);

		try {
			List<Usluga> filterUsluga = listaUsluga.stream()
					.filter(usluga -> usluga.getNaslov().toLowerCase().contains(nazivString.toLowerCase()))
					.filter(usluga -> usluga.getOpis().toLowerCase().contains(opisString.toLowerCase()))
					.filter(usluga -> usluga.getCijena().toString().toLowerCase().contains(cijenaString.toLowerCase()))
					.collect(Collectors.toList());

			dataU = FXCollections.observableList(filterUsluga);
